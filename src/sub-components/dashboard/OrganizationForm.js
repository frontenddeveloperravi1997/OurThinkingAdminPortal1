...
const OrganizationForm = ({ 
    ...
    topicList,
    languageList,
    method ,org,orgKeyContactList,comparativeList}) => {
...
const {
  ...
  mutationFn: async (data) => {
    return await commonQuery("POST", `/api/Organization/CreateOrganization`, data);
  },
...
const {
  ...
  mutationFn: async (data) => {
    return await commonQuery("PUT", `/api/Organization/UpdateOrganization`, data);
  },
  onSuccess(data, variables, context) {
    if(data?.data?.statusCode == 200){
      setOrgCreated(true)
  ...
  const {
    ...
    defaultValues: {
        ...
        isInternalOrg:{ value: org?.internalOrganization?true:false, label: org?.internalOrganization?"Yes":"No" },
        isAutoLogin: { value: org?.autoLogin?true:false, label: org?.autoLogin?"Yes":"No"  },
...
<CommonModal show={showCancelPop} onClose={handleHideCancelPop} heading={"Cancel changes?"} body={"All the save changes will be lost"}>
  <>
      ...
      <Row className="mb-8">
        <Col md={{ offset: 3, span: 11 }} xs={12} className="mt-2 d-flex gap-4">
          ...
          >
            ...
            {(isPending || isEditPending) && <Spinner style={{marginLeft:"8px"}} animation="border"  size="sm"
          role="status"
          aria-hidden="true" />}
